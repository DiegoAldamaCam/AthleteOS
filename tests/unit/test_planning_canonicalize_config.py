"""Unit tests for PlanningCanonicalizeJobConfig input validation (RISK F7 — DDL injection).

These tests run WITHOUT pyflink (import-safe by design). They verify that
PlanningCanonicalizeJobConfig rejects values containing quote/newline/property-injection
...
Scenarios covered: sc-13 (forbidden chars rejected on each field),
sc-14 (clean values accepted / happy path).
"""
...
def _make(**overrides) -> PlanningCanonicalizeJobConfig:
    kwargs = dict(_GOOD)
    kwargs.update(overrides)
    return PlanningCanonicalizeJobConfig(**kwargs)


# ---------------------------------------------------------------------------
# Happy-path: valid values must construct without error (sc-14)
# ---------------------------------------------------------------------------


class TestPlanningCanonicalizeJobConfigValid:
    ...
    def test_canonical_topic_accepted(self):
        cfg = _make(canonical_topic="canonical.planning_block.v2")
        assert cfg.canonical_topic == "canonical.planning_block.v2"


# ---------------------------------------------------------------------------
# Injection rejection: sc-13 — forbidden chars on each validated field
# ---------------------------------------------------------------------------


class TestPlanningCanonicalizeJobConfigRejectsInjection:
    ...
    def test_single_quote_in_canonical_topic_rejected(self):
        with pytest.raises(ValueError, match="canonical_topic"):
            _make(canonical_topic="canonical.planning_block'")

    def test_double_quote_in_canonical_topic_rejected(self):
        with pytest.raises(ValueError, match="canonical_topic"):
            _make(canonical_topic='canonical.planning_block"injected')
