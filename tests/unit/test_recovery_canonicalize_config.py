"""Unit tests for RecoveryCanonicalizeJobConfig input validation (RISK F5 — DDL injection).

These tests run WITHOUT pyflink (import-safe by design). They verify that
RecoveryCanonicalizeJobConfig rejects values containing quote/newline/property-injection
...
Scenarios covered: sc-9 (forbidden chars rejected on each field),
sc-10 (clean values accepted / happy path).
"""
...
from jobs.recovery_canonicalize.main import RecoveryCanonicalizeJobConfig
...
def _make(**overrides) -> RecoveryCanonicalizeJobConfig:
    kwargs = dict(_GOOD)
    kwargs.update(overrides)
    return RecoveryCanonicalizeJobConfig(**kwargs)


# ---------------------------------------------------------------------------
# Happy-path: valid values must construct without error (sc-10)
# ---------------------------------------------------------------------------


class TestRecoveryCanonicalizeJobConfigValid:
...
# ---------------------------------------------------------------------------
# Injection rejection: sc-9 — forbidden chars on each validated field
# ---------------------------------------------------------------------------


class TestRecoveryCanonicalizeJobConfigRejectsInjection:
