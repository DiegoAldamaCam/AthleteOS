"""Unit tests for the recovery CSV watcher (PR-R1).
...
Spec scenarios: sc-10 (sorted file processing), sc-11 (graceful shutdown).
"""
...
from ingestion.recovery.watcher import (
    ProcessingSummary,
    process_csv_file,
    process_directory,
    watch_directory,
...
_VALID_ROW_1 = "A1,2025-06-01,7.5,58,42.0,8500,72.3\n"
_VALID_ROW_2 = "A2,2025-06-02,8.0,55,45.0,9000,70.0\n"
...
class _RecordingPublisher:
    ...
    def publish(self, record: RecoveryRecord, **_kwargs) -> str:
...
def test_process_csv_file_publishes_all_valid_rows(tmp_path):
    """A fully-valid recovery CSV publishes every row and flushes the publisher."""
    ...
    assert pub.published_records[0].hrv == 42.0
...
def test_process_csv_file_skips_malformed_rows_and_keeps_valid(tmp_path):
    ...
    csv_path.write_text(
        _HEADER
        + _VALID_ROW_1  # valid
        + ",2025-06-02,7.5,58,42.0,8500,72.3\n"  # empty athlete_id
        + _VALID_ROW_2  # valid
    )
...
# --- process_directory (sc-10) ---


def test_process_directory_processes_csv_files_in_sorted_order(tmp_path):
    """sc-10: a.csv MUST be processed before b.csv (sorted order)."""
...
def test_process_directory_returns_summary_per_file(tmp_path):
    """sc-10 triangulation: one ProcessingSummary returned per CSV file."""
    (tmp_path / "a.csv").write_text(_HEADER + _VALID_ROW_1)
    (tmp_path / "b.csv").write_text(_HEADER + _VALID_ROW_2)
...
# --- watch_directory (sc-11) ---


def test_watch_directory_exits_when_stop_event_set(tmp_path):
    """sc-11: watch_directory exits after stop_event.set() is called."""
    (tmp_path / "a.csv").write_text(_HEADER + _VALID_ROW_1)
    pub = _RecordingPublisher()
...
def test_watch_directory_no_further_files_processed_after_stop(tmp_path):
    """sc-11 triangulation: after stop_event is set, no new files processed."""
    (tmp_path / "a.csv").write_text(_HEADER + _VALID_ROW_1)
    pub = _RecordingPublisher()
    stop_event = threading.Event()
