...
# with a ValueError before it can reach tbl_env.execute_sql. (RISK F3)
...
def _validate_ddl_config_field(field_name: str, value: str) -> None:
    ...
    config parameter is problematic. Called in WellnessCanonicalizeJobConfig.__init__
    for every field that is interpolated into the sink DDL f-string. (RISK F3)
    """
    for ch in _DDL_FORBIDDEN_CHARS:
        if ch in value:
            raise ValueError(
                f"WellnessCanonicalizeJobConfig.{field_name} contains a character that is "
...
class WellnessCanonicalizeJobConfig:
    ...
    def __init__(
        ...
        *,
        ...
        # DDL injection guard (RISK F3): validate before any field assignment.
