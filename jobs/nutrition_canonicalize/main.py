"""PyFlink nutrition canonicalize job wiring (PR-N2).
...
:mod:`jobs.nutrition_canonicalize.transform`, which imports WITHOUT pyflink and is
fully unit-tested (tests/unit/test_nutrition_transform.py).

The integration slice (tests/integration/test_nutrition_canonicalize_job.py)
...
Job topology (design #241)
==========================
raw.nutrition → canonical.wellness_event (NUTRITION_DAILY events):
    ...
    KafkaSource(raw.nutrition, SimpleStringSchema-JSON)
      .assign_watermark(WatermarkStrategy.for_bounded_out_of_orderness(24h)
        ...
        # raw.nutrition envelope (W1-5 compliant: producer emits epoch-ms longs).
      -> key_by(event_id)           # dedup keyed by event_id (LOCKED)
      -> .process(NutritionCanonicalizeProcessFunction, output_type=...)
      ...
      [side] -> KafkaSink(dlq.canonical.wellness_event, JSON, AT_LEAST_ONCE)
                (original_topic="raw.nutrition" disambiguates from wellness/recovery DLQ)

ADR-N3: transactional_id_prefix = "athleteos-canonicalize-nutrition-wellness-event"
=========================================================================
Disjoint from:
...
ProducerFencedException between nutrition, recovery and wellness jobs writing the
same canonical.wellness_event topic (sc-25).

ADR-N1: event_type = NUTRITION_DAILY (hardcoded in transform.py, not read
from payload). Verified at the unit layer by TRANSACTIONAL_ID_PREFIX test (sc-25)
and the transform unit tests (sc-16, sc-17).
"""
...
RAW_TOPIC = "raw.nutrition"
CANONICAL_TOPIC = "canonical.wellness_event"
DLQ_TOPIC = "dlq.canonical.wellness_event"
SOURCE_NAME = "nutrition-canonicalize-source"

# ADR-N3: distinct transactional_id_prefix — disjoint from strength/wellness/planning/cardio/recovery.
# sc-25 asserts this EXACT literal string; do NOT change without updating the test.
TRANSACTIONAL_ID_PREFIX = "athleteos-canonicalize-nutrition-wellness-event"

# Dedup ValueState<bool> per event_id, 7d TTL (LOCKED design, mirrors wellness/cardio/recovery).
DEDUP_TTL_DAYS = 7
# Bounded out-of-orderness: 24h (nutrition uploads land late, same as wellness/cardio/recovery).
...
def _validate_ddl_config_field(field_name: str, value: str) -> None:
    for ch in _DDL_FORBIDDEN_CHARS:
        if ch in value:
            raise ValueError(
                f"NutritionCanonicalizeJobConfig.{field_name} contains a character that is "
...
class NutritionCanonicalizeJobConfig:
    """Plain configuration container (no pyflink). Import-safe.

    Mirrors RecoveryCanonicalizeJobConfig / CardioCanonicalizeJobConfig from the
    sibling jobs, with nutrition-specific topic defaults.
    """
    ...
    def __init__(
        ...
        *,
        ...
        group_id: str = "canonicalize-nutrition",
...
def run(config: NutritionCanonicalizeJobConfig) -> None:  # pragma: no cover - flink runtime
    """Build and execute the nutrition canonicalize job.
    ...
    # --- source: raw.nutrition as JSON string --------------------------------
    source_builder = (
        KafkaSource.builder()
    ...
    # raw.nutrition JSON envelope. The nutrition producer emits epoch-ms longs (W1-5).
    ...
    # 22 columns — identical to the wellness/recovery canonicalize job DDL).
    ...
    class NutritionCanonicalizeProcessFunction(KeyedProcessFunction):  # type: ignore[misc]
        ...
        Row; duplicate → silently dropped (sc-23). ValidationError / TransformError
        → DLQ side output (sc-21, sc-22). event_type is ALWAYS NUTRITION_DAILY
        (hardcoded in transform.py — ADR-N1).
        ...
        def open(self, runtime_context: Any) -> None:
            ...
            self._seen = runtime_context.get_state(
                ValueStateDescriptor("seen-nutrition-event-id", Types.BOOLEAN())
            )
        ...
        def process_element(self, value: str, ctx: Any) -> None:
            ...
            if bool(self._seen.value()):
                return  # duplicate within 7d re-delivery window → silently dropped (sc-23)
            ...
            try:
                canonical = transform_nutrition_to_canonical(raw, self._schema_version)
    ...
    # original_topic="raw.nutrition" in the DLQ envelope disambiguates from
    # the wellness job's DLQ records (original_topic="raw.wellness") and the
    # recovery job's DLQ records (original_topic="raw.recovery") at triage time.
    ...
    # ADR-N3: transactional_id_prefix is DISTINCT from wellness/recovery/cardio jobs to
    # prevent ProducerFencedException when all jobs write to canonical.wellness_event
    # with EXACTLY_ONCE semantics (sc-25).
    # ADR-16 (wellness): event_type is declared as STRING (not enum); the
    # nutrition job inherits this DDL convention from the wellness DDL.
    sink_ddl = f"""
CREATE TABLE canonical_nutrition_wellness_event_sink (
  `event_id` STRING,
  `event_time` BIGINT,
  `ingest_time` BIGINT,
  `source` STRING,
...
"""
    ...
    statement_set.add_insert("canonical_nutrition_wellness_event_sink", canonical_table)
    statement_set.attach_as_datastream()

    env.execute("athleteos-nutrition-canonicalize-job")


def main() -> int:  # pragma: no cover - entrypoint
    bootstrap = os.environ.get("KAFKA_BOOTSTRAP_SERVERS", "kafka:9092")
    registry = os.environ.get("SCHEMA_REGISTRY_URL", "http://schema-registry:8081")
    group = os.environ.get("NUTRITION_CANONICALIZE_GROUP_ID", "canonicalize-nutrition")
