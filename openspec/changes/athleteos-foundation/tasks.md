...
- [x] 4.3 Integration test (HIGHEST RISK): raw JSON → canonical Avro roundtrip via Schema Registry + DLQ routing on invalid input + dedup drops duplicate event_ids (testcontainers Kafka + Schema Registry) — RUNTIME-VERIFIED (live Redpanda + Flink 1.19, apache-flink 1.19.1 on CPython 3.11): tests/integration/test_canonicalize_job.py PASSED. Real integration bug surfaced + fixed (Confluent SR HTTP 409 schema-compat: Flink avro-confluent Table format cannot reproduce the avsc `event_type` enum; live sink now owns its writers schema). Details: Engram obs #30, topic sdd/athleteos-foundation/apply-progress.
