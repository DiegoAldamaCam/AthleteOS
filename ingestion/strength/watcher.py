"""File/directory watcher for the strength ingestion connector (PR2, 3.1).

``process_csv_file`` is the core: it reads a Strong CSV file, parses every row
via ``parse_csv`` (skip-and-collect), publishes each valid record to
``raw.strength`` through the injected publisher, and flushes once at the end.

``process_directory`` scans a directory for ``*.csv`` files and processes each
once. ``watch_directory`` wraps that in a polling loop (no extra dependency -
uses ``threading.Event`` for responsive shutdown) so the connector can tail a
drop folder in production.
...
integration test wires a real ``StrengthPublisher`` against testcontainers Kafka.
"""
...
from ingestion.strength.parser import parse_csv

_DEFAULT_POLL_INTERVAL = 5.0


class _Publisher(Protocol):
    """Structural type matching StrengthPublisher (publish + flush)."""
...
def process_csv_file(
    ...
    """Parse a Strong CSV file and publish every valid record; skip malformed.

    Opens the file as UTF-8 with ``newline=""`` (csv module requirement), reads
    via ``DictReader`` (header names per the spec source-field mapping), and
    publishes each parsed record. The publisher is flushed exactly once before
    returning so queued records are delivered.
    """
...
def watch_directory(
    ...
    event is observed promptly rather than after a full sleep. Intended to run
    in the ingestion container (``ingest`` compose profile).
    """
