"""File/directory watcher for the wellness ingestion connector (PR-W1).

``process_csv_file`` reads a wellness CSV file, parses every row via
``parse_csv`` (skip-and-collect), publishes each valid record to
``raw.wellness`` through the injected publisher, and flushes once at the end.
...
integration test wires a real ``WellnessPublisher`` against testcontainers Kafka.

Mirrors ``ingestion/strength/watcher.py`` symbol-for-symbol.
"""
...
from ingestion.wellness.parser import parse_csv

_DEFAULT_POLL_INTERVAL = 5.0


class _Publisher(Protocol):
    """Structural type matching WellnessPublisher (publish + flush)."""
...
def process_csv_file(
    ...
    """Parse a wellness CSV file and publish every valid record; skip malformed.
