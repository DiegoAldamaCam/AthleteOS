"""File/directory watcher for the nutrition ingestion connector (PR-N1).

``process_csv_file`` reads a nutrition CSV file, parses every row via
``parse_csv`` (skip-and-collect), publishes each valid record to
``raw.nutrition`` through the injected publisher, and flushes once at the end.
...
integration test wires a real ``NutritionPublisher`` against testcontainers Kafka.

Mirrors ``ingestion/recovery/watcher.py`` symbol-for-symbol.
"""
...
from ingestion.nutrition.parser import parse_csv

_DEFAULT_POLL_INTERVAL = 5.0


class _Publisher(Protocol):
    """Structural type matching NutritionPublisher (publish + flush)."""
...
def process_csv_file(
    ...
    """Parse a nutrition CSV file and publish every valid record; skip malformed.
