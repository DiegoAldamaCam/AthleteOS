"""File/directory watcher for the cardio ingestion connector (PR-C1).

``process_csv_file`` reads a cardio CSV file, parses every row via
``parse_csv`` (skip-and-collect), publishes each valid record to
``raw.cardio`` through the injected publisher, and flushes once at the end.
...
integration test wires a real ``CardioPublisher`` against testcontainers Kafka.
...
from ingestion.cardio.parser import parse_csv

_DEFAULT_POLL_INTERVAL = 5.0


class _Publisher(Protocol):
    """Structural type matching CardioPublisher (publish + flush)."""
...
def process_csv_file(
    ...
    """Parse a cardio CSV file and publish every valid record; skip malformed.
